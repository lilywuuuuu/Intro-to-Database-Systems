    ...
    if answer1 == "1":
        ...
        if answer2 == "1":
            ...
            cur.execute("select code_name._name, code_name._code, list_co._group \
                        from list_co, code_name where list_co._group like '%" \
                        + group_name + "%' and code_name._code = list_co._code")
            results = cur.fetchall()
            for row in results:
                count += 1
        ...
        if("1100" < company_input < "9999"):
            ...
            cur.execute("select _name from code_name \
                where _code = '" + company_code + "'")
            company_name = cur.fetchall()[0]["_name"]
        else:
            company_name = company_input
            cur.execute("select _name, _code from code_name \
                where _name like '%" + company_name + "%'")
            result = cur.fetchall()
